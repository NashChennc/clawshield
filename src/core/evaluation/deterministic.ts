...
import type { RetrievedPolicy } from "../policy/retriever.js";
...
const SHELL_BLOCK_MARKERS = ["rm -rf", "del /f /s /q", "format ", "curl ", "wget ", "powershell -enc"];
...
const SENSITIVE_READ_MARKERS = ["get-content", "cat ", "type ", "select-string", "grep ", "findstr "];

const EXFIL_MARKERS = ["curl ", "wget ", "scp ", "rsync ", "invoke-webrequest", "ftp "];
...
function pyLikeText(value: unknown): string {
  if (value == null) return "";
  if (typeof value === "string") return value;
  if (typeof value === "number" || typeof value === "boolean" || typeof value === "bigint") return String(value);
  if (Array.isArray(value)) return value.map((v) => pyLikeText(v)).join(" ");
  if (typeof value === "object") {
    return Object.entries(value as Record<string, unknown>)
      .map(([k, v]) => `${k}: ${pyLikeText(v)}`)
      .join(" ");
  }
  return String(value);
}
...
export function inspectHardBarrier(event: Event, retrieved: RetrievedPolicy[], runtimeDir: string): Decision | null {
  if (event.event_type === "tool_result") {
    const contentText = pyLikeText(event.payload.content).toLowerCase();
  ...
  if (event.event_type === "tool_call_attempt" && event.tool_name === "shell") {
    ...
    if (SENSITIVE_SOURCE_MARKERS.some((m) => command.includes(m))) {
      ...
      if (SENSITIVE_READ_MARKERS.some((m) => command.includes(m)) || command.includes("copy ") || command.includes("move ")) {
        return new Decision({
...
export function inspectFallbackHeuristics(event: Event, retrieved: RetrievedPolicy[]): Decision | null {
  ...
  if (event.event_type === "user_request") {
    const summary = `${event.summary.toLowerCase()} ${pyLikeText(event.payload.content).toLowerCase()}`;
