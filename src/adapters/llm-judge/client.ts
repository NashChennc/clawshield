import { Decision } from "../../core/models/decision.js";
...
export class GuardJudgeClient {
  ...
  async generatePolicyCandidates(context: Record<string, unknown>): Promise<Array<Record<string, unknown>> | null> {
    if (!this.configured()) return null;
    const systemPrompt =
    ...
    try {
      ...
      if (!Array.isArray(policies)) throw new Error("policies must be a list");
      return policies as Array<Record<string, unknown>>;
    } catch {
      return null;
    }
  }
