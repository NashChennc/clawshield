...
import { normalizeOpenclawToolName } from "../openclaw-normalize.js";
...
export async function routeHookEval(
  ...
  if (hook === "before_prompt_build") {
    return adapter.beforePromptBuild(payload, sessionId);
  }
  ...
  if (hook === "tool_result_persist") {
    return adapter.toolResultPersist(payload, sessionId);
  }
  return {
    action: "allow",
    risk_score: 0,
