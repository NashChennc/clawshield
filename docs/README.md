...
1. `before_prompt_build` / `before_tool_call` / `after_tool_call` 在 **同一 Node 进程** 内 `await SafetyCore.evaluate`。
2. `tool_result_persist` 为 **同步**钩子：插件通过 **`spawnSync`** 启动同包内的 **`dist/eval-once.js`**，stdin 传入 `{ hook, payload, session_id }`，子进程跑完完整评估后 **stdout 仅输出裁决 JSON**（与原先阻塞式 `hook-eval` 语义一致）。
...
| 钩子 | 行为 |
|------|------|
| `before_prompt_build` | 注入 ClawShield 系统说明；可按配置对当前 prompt / 消息再跑 SafetyCore |
...
- **`enablePromptContextEval`**：是否在 `before_prompt_build` 跑 SafetyCore（默认 true）。
